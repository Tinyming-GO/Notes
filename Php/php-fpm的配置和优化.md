...
官方文档：https://php.net/manual/zh/install.fpm.configuration.php
...
`pm = static` 表示我们创建的php-fpm子进程数量是固定的，那么就只要 `pm.max_children = 50` 这个参数生效。你启动php-fpm的时候就会一起全部启动51(1个主＋50个子)个进程，颇为壮观。
...
当nginx收到如上错误码时，可以确定后端php-fpm解析php出了某种问题，比如，执行错误，执行超时。这个时候，我们是可以开启慢日志功能的。
